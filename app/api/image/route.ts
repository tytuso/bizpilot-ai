...
export async function POST(request:NextRequest){
  try{
    ...
    const response=await fetch("https://api.openai.com/v1/images/generations",{method:"POST",headers:{Authorization:`Bearer ${key}`,"Content-Type":"application/json"},body:JSON.stringify({model:process.env.IMAGE_MODEL||"gpt-image-2",prompt:`Create a premium small-business advertisement. ${prompt}. No invented logos, claims, phone numbers or prices beyond those explicitly provided. Keep visible typography minimal and accurate.`,size:body?.size||"1024x1024",quality:"low",output_format:"png"})});
